...
print(f"  【複合語別使用頻度（トップ10）】")
for compound, count in sorted(compound_usage.items(), key=lambda x: -x[1])[:10]:
...
for result in validation_results:
    ...
    if expected == actual:
        print(f"  ✓ {result['problem_id']:20} - 完全一致")
    elif expected.issubset(actual):
        print(f"  △ {result['problem_id']:20} - 追加キーワード: {list(actual - expected)}")
    elif expected.issuperset(actual):
        print(f"  ✗ {result['problem_id']:20} - 欠落キーワード: {list(expected - actual)}")
    else:
        print(f"  ✗ {result['problem_id']:20} - 不一致: {list(expected ^ actual)}")
...
report_path = "output/validation_report_compound_words.json"
...
🔍 主要複合語使用状況：
  ...
  - その他: {sum(v for k, v in compound_usage.items() if k not in ['営業許可', '型式検定', '遊技機'])}問
