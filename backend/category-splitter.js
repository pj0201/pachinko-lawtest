 ...
 *
 * 目的: 7カテゴリを2グループに分割
 * - Worker3 グループ: カテゴリ 1-3.5 (750問)
 * - Worker2 グループ: カテゴリ 3.5-7 (750問)
 *
 * 使用方法:
 * node category-splitter.js worker3  → Worker3用スクリプト生成
 * node category-splitter.js worker2  → Worker2用スクリプト生成
 */
...
const SPLITS = {
  worker3: {
    name: 'Worker3',
    categories: [
      ALL_CATEGORIES[0],  // 営業許可・申請手続き
      ALL_CATEGORIES[1],  // 建物・設備基準
      ALL_CATEGORIES[2],  // 従業員・管理者要件
      // 営業時間は分割: 前半部分
      'split:営業時間・休業日管理(前半)'
    ],
    target_problems: 750,
    description: 'カテゴリ 1-3.5 (基本・設備・従業員・営業時間前半)'
  },
  worker2: {
    name: 'Worker2',
    categories: [
      // 営業時間は分割: 後半部分
      'split:営業時間・休業日管理(後半)',
      ALL_CATEGORIES[4],  // 景品・景慮基準
      ALL_CATEGORIES[5],  // 法律・規制違反・処分
      ALL_CATEGORIES[6]   // 実務・業務管理・記録
    ],
    target_problems: 750,
    description: 'カテゴリ 3.5-7 (営業時間後半・景品・法律・実務)'
  }
};
...
function generateWorkerScript(worker, ANTHROPIC_API_KEY_PLACEHOLDER) {
  const config = SPLITS[worker];
  const categoriesJson = JSON.stringify(config.categories, null, 2);

  return `#!/usr/bin/env node

/**
 * 最高品質問題生成エンジンv2 - ${config.name}専用版
 *
 * 目的: ${config.description}
 * 目標問題数: ${config.target_problems}問
 *
 * 実行方法:
 * export ANTHROPIC_API_KEY="sk-ant-..."
 * node ultimate-problem-generator-v2-${worker}.js
 */
...
const CONFIG = {
  ...
  outputPath: path.join(__dirname, '../data/ultimate_problems_${worker}.json'),
  learningPath: path.join(__dirname, '../data/learning_stats_${worker}.json'),
  targetProblems: ${config.target_problems},
...
class ProblemGenerator {
  ...
  async generateAll() {
    console.log('🚀 ${config.name}: 生成エンジン起動');
...
if (!worker || !['worker2', 'worker3'].includes(worker)) {
  console.log('🚀 カテゴリ分割スクリプト生成ツール');
  console.log();
  console.log('使用方法:');
  console.log('  node category-splitter.js worker3  → Worker3用スクリプト生成');
  console.log('  node category-splitter.js worker2  → Worker2用スクリプト生成');
  console.log();
  console.log('生成内容:');
  console.log('  Worker3: ultimate-problem-generator-v2-worker3.js');
  console.log('           カテゴリ 1-3.5 (750問)');
  console.log('  Worker2: ultimate-problem-generator-v2-worker2.js');
  console.log('           カテゴリ 3.5-7 (750問)');
  process.exit(1);
}

const config = SPLITS[worker];
const script = generateWorkerScript(worker, 'sk-ant-...');
const outputPath = path.join(__dirname, `ultimate-problem-generator-v2-${worker}.js`);

fs.writeFileSync(outputPath, script);

console.log(`✅ スクリプト生成完了: ${outputPath}`);
console.log();
console.log(`📋 ${config.name} 専用スクリプト情報:`);
console.log(`   説明: ${config.description}`);
console.log(`   目標: ${config.target_problems}問`);
console.log(`   カテゴリ数: ${config.categories.length}`);
console.log();
console.log(`📂 カテゴリ構成:`);
...
console.log(`   node ${path.basename(outputPath)}`);
console.log();
console.log(`⚠️  注意: ANTHROPIC_API_KEY を設定してから実行してください`);
