...
def extract_from_database(db_path):
    ...
    # WIND_BUSINESS_LAW オブジェクトを抽出
    match = re.search(r'export const WIND_BUSINESS_LAW\s*=\s*({[\s\S]*?});?\s*$', content, re.MULTILINE)
    if not match:
        print("Error: WIND_BUSINESS_LAW not found")
        return {}

    law_obj_str = match.group(1)
