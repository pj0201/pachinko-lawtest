...
def extract_from_database(db_path):
    ...
    # lawDatabaseの配列部分を抽出
    # 配列は非常に大きいので、風営法のセクションだけを抽出
    # まず風営法のオブジェクトを見つける
    # lawName: "風俗営業等の規制及び業務の適正化等に関する法律" を含む部分を探す

    # 手動でパースする
    # まず lawName を含む行を探す
    law_start = content.find('"風俗営業等の規制及び業務の適正化等に関する法律"')
    if law_start == -1:
        print("Error: 風営法 not found in database")
        return {}

    # この法律のオブジェクトの開始位置を探す（{ で始まる）
    brace_start = content.rfind('{', 0, law_start)
    if brace_start == -1:
        print("Error: Cannot find start of law object")
        return {}
    ...
    if brace_end == -1:
        print("Error: Cannot find end of law object")
        return {}

    law_obj_str = content[brace_start:brace_end]

    # JSONとして解析
    try:
        fueihou = json.loads(law_obj_str)
    except Exception as e:
        print(f"Error parsing law object: {e}")
        return {}
